...
if ispc
    ...
    figure_save_folder = "C:\Users\mdsaifi\Box\Research\Semesters\Fall2024\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\chronological_plots\mag_phase";
elseif ismac
...
pattern = '^\d';  % Regular expression to match strings that start with a digit
idx = ~cellfun('isempty', regexp(x, pattern));
folders_to_check = x(idx);
cellNumber = 1;
% load('temporal_data_1.mat');
...
% ii=1;
    ...
    for jj=1:length(data)
        if temporal_data(ii).frequency == data(jj, 1)
            scatter(s11a(jj), s11m(jj), 50, data(jj, 7:9), 'filled', 'marker', 'o');
            hold on;
