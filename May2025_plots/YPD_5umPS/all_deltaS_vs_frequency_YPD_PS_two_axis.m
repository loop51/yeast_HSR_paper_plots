...
plot_line_width = 2;
if ispc
    experiment_folder = "C:\Users\mdsaifi\Box\Measurement\Fall2024\September19th";
    figure_save_folder = "k:\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\March2025_v2\YPD_5um_PS\deltaS_vs_frequency";
elseif ismac
    experiment_folder = '/Users/ruzvaysaiful/Library/CloudStorage/Box-Box/Measurement/Fall2024/September19th';
    figure_save_folder = '/Users/ruzvaysaiful/Library/CloudStorage/Box-Box/Research/Semesters/Fall2024/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/YPD_5um_PS/deltaS_vs_frequency';
elseif isunix && ~ismac
    experiment_folder = '/media/mdsaifi/Grace Lab/measurement/fall2024/september19th';
    figure_save_folder = '/media/mdsaifi/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/YPD_5um_PS/deltaS_vs_frequency';
end
...
% assign_set_32C = [1, 5];
assign_iteration_32C = ones(5,1)';
assign_set_32C = linspace(1,5, 5);

% assign_iteration_42C = [1,1, 1, 1, 1];
% assign_set_42C = [2,5, 9, 15, 25];
assign_iteration_42C = ones(25,1)';
assign_set_42C = linspace(1,25,25);

assign_iteration_42C(10) =[];
assign_set_42C(10) = [];
% 
assign_iteration_42C(10) =[];
assign_set_42C(10) = [];

assign_iteration_42C(20) =[];
assign_set_42C(20) = [];

assign_iteration_42C(21) =[];
assign_set_42C(21) = [];
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<4
        plot(f, deltaS.s11m(i,:) - deltaS.s11m(1,:), '-o', 'markersize', 3, 'Color', colors(i,:), 'LineWidth', plot_line_width);
    elseif i>=3 && i<6
        plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), ':s', 'markersize', 3, 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), '--o','markersize', 3,'Color', colors(i,:), 'LineWidth', plot_line_width);
    end
end
...
% end
xlabel('{\it f (GHz)}');
% ylabel('\Delta |S11| - \Delta |S11|_{t=0}');
ylabel('\delta |S11|');
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<3
        plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '-s', 'markersize', 3, 'Color', colors(i,:), 'LineWidth', plot_line_width);
    elseif i>=3 && i<6
        plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), ':s', 'markersize', 3, 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '--s', 'markersize', 3,'Color', colors(i,:),'LineWidth', plot_line_width);
...
xlabel('{\it f (GHz)}');
% ylabel('\Delta \angle S11 - \Delta \angle S11_{t=0}');
ylabel('\delta \angle S11');
plot_s11a = gca;

    current_plot = gca;
    current_plot.YColor = 'k';
...
%     hold on;
%     if i<4
%         plot(f, deltaS.s21m(i,:)-deltaS.s21m(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
%     else
%         plot(f, deltaS.s21m(i,:)-deltaS.s21m(1,:), '--o','Color', colors(i,:), 'LineWidth', plot_line_width);
%     end
% end
...
% ylim([-5e-3 0.03]);
% xlabel('f (GHz)');
% ylabel('\Delta |S21| - \Delta |S21|_{t=0}');
% plot_s21m = gca;
% 
% figure_s21a = figure();
% for i=1:size(deltaS.s11m,1)
%     hold on;
%     if i<4
%         plot(f, deltaS.s21a(i,:)-deltaS.s21a(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
%     else
%         plot(f, deltaS.s21a(i,:)-deltaS.s21a(1,:), '--o','Color', colors(i,:), 'LineWidth', plot_line_width);
%     end
% end
...
% ylim([-5e-3 0.03]);
% xlabel('f (GHz)');
% ylabel('\Delta \angle S21 - \angle |S21|_{t=0}');
% plot_s21a = gca;
%%
figure_dimension = 3.5;
figure_s11m.Units = 'inches';
figure_s11m.Position = [21.3229 0.4583 figure_dimension+1 figure_dimension];

% figure_s11m.OuterPosition = [21.3229 0.4583 figure_dimension figure_dimension];
figure_s11m.MenuBar = 'none';
plot_s11m.FontSize = 8;
% plot_s11m.LineWidth = 2;

exportgraphics(figure_s11m,strcat(figure_save_folder, filesep, 'all_reflection.pdf'), 'ContentType','vector');
exportgraphics(figure_s11m,strcat(figure_save_folder, filesep, 'all_reflection.png'), 'ContentType','vector');
% 
% figure_s11a.Units = 'inches';
% figure_s11a.Position = [21.3229 0.4583 figure_dimension figure_dimension];
...
% figure_s21m.Position = [21.3229 0.4583 figure_dimension figure_dimension];
...
% figure_s21a.Position = [21.3229 0.4583 figure_dimension figure_dimension];
