...
clear;
close all;
%Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/DI_Water_5um_PS/deltaS_vs_time
...
if ispc
    ...
    figure_save_folder = "K:\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\March2025_v2\YPD_5um_PS\deltaS_vs_time";
elseif ismac
    experiment_folder = '/Users/ruzvaysaiful/Library/CloudStorage/Box-Box/Measurement/Fall2024/September19th';
    figure_save_folder = '/Users/ruzvaysaiful/Library/CloudStorage/Box-Box/Research/Semesters/Fall2024/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/YPD_5um_PS/deltaS_vs_time';
elseif isunix && ~ismac
    experiment_folder = '/media/mdsaifi/Grace Lab/measurement/fall2024/september19th';
    figure_save_folder = '/media/mdsaifi/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/YPD_5um_PS/deltaS_vs_time';
end
...
% for ii=1:size(temporal_data,2)
    ...
    yt = current_plot.YTick;
    % yticks(linspace(min(yt), max(yt), 6)); % Set 8 ticks between min and max
    yticks(min(yt):0.01:max(yt));
    yyaxis left;
    current_plot = gca;
    ...
    filename = strcat('reflection', num2str(temporal_data(ii).frequency), "GHz",'.pdf');
    current_figure = gcf;
    % exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
...
%     fill([x fliplr(x)], [y zeros(size(y))], 'r', 'FaceAlpha', 0.3);
    ...
    % text(mean(x)-1.8*std(x), mean(y)-4*std(y), '@32C');
    x =x_fit(x_fit<=for_transition_x);


    xline(max(x), '--r');
    x =x_fit(x_fit>=for_transition_x);
    % text(mean(x), mean(y)-4*std(y), '@42C');
