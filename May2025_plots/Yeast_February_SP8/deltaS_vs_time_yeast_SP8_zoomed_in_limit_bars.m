...
plot_line_width = 2.5;
if ispc
    experiment_folder = "C:\Users\mdsaifi\Box\Measurement\spring2025\yeast_temperature_experiment_repeat\yeast_SP8";
    figure_save_folder = "k:\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\March2025_v2\yeast_SP8\deltaS_vs_time";
elseif ismac
...
elseif isunix && ~ismac
    experiment_folder = '/media/mdsaifi/Grace Lab/measurement/spring2025/yeast_temperature_experiment_repeat/yeast_SP8';
    figure_save_folder = '/media/mdsaifi/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/yeast_SP8/deltaS_vs_time';
end
...
for ii=1:size(temporal_data,2)
  temporal_data(ii).data = sortrows(temporal_data.data, 5);
...
% for ii=1:size(temporal_data,2)
    % S11m
    % temporal_data(ii).data(:,5) = sortrows(temporal_data(ii).data(:,5));
    ...
    ax = gca;
    ax.YAxis.Exponent = -2;
    filename = strcat('zoomed_in_S11m_', num2str(temporal_data(ii).frequency), "GHz",'.pdf');
    exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
    filename = strcat('zoomed_in_S11m_', num2str(temporal_data(ii).frequency), "GHz",'.png');
    % exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
    ...
    ax = gca;
    ax.YAxis.Exponent = -2;
    filename = strcat('zoomed_in_S11a_', num2str(temporal_data(ii).frequency), "GHz",'.pdf');
    exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
    filename = strcat('zoomed_in_S11a_', num2str(temporal_data(ii).frequency), "GHz",'.png');
    % exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
    ...
    max_s21m = max(y(idx));
    min_s21m = min(y(idx));
    diff_s21m = max_s21m - min_s21m;
    
    deltaS_vs_time_plot(x,y, for_32C_x,for_transition_x);
    ylabel('\Delta |S21|');
    % title(strcat("@", num2str(temporal_data(ii).frequency),'GHz'))
    xlim([ll upl]);
    filename = strcat('zoomed_in_S21m_', num2str(temporal_data(ii).frequency), "GHz",'.pdf');
    current_figure = gcf;
    % exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
    ...
    ax = gca;
    ax.YAxis.Exponent = -1;
    filename = strcat('zoomed_in_S21a_', num2str(temporal_data(ii).frequency), "GHz",'.pdf');
    exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');

    filename = strcat('zoomed_in_S21a_', num2str(temporal_data(ii).frequency), "GHz",'.png');
    % exportgraphics(current_figure,strcat(figure_save_folder, filesep, filename), 'ContentType','vector');
% end
...
%     % y_32C = y;
    ...
    x =x_fit(x_fit<=for_transition_x);
    time_points = [0 10 30 50 92 122 150];
