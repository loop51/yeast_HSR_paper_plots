...
if ispc
    ...
    figure_save_folder = 'K:\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\March2025_v2\september20th\Choronology_Plot\may';
elseif ismac
...
% ii=4;
    ...
    xlabel('\Delta\angle S_{11}', 'FontAngle', 'italic');
    ylabel('\Delta|S_{11}|', 'FontAngle', 'italic');
    
    current_figure = gcf;
    current_plot = gca;
    current_figure.Units = 'inches';
    current_figure.Position = [21.3229 0.4583 2.75 2.75];
...
% ii=1;
    ...
    for jj=1:length(data)
        if temporal_data(ii).frequency == data(jj, 1)
            ...
            elapsed_time = minutes(datetime(data(jj, 2), 'convertfrom', 'datenum') - experiment_start_time);
            if elapsed_time < 50
                scatter(s21a(jj), s21m(jj), 20, data(jj, 7:9), 'filled', 'marker', 'o');
            elseif elapsed_time >50 && elapsed_time < 80
                scatter(s21a(jj), s21m(jj), 20, data(jj, 7:9), 'marker', 'o');
            elseif elapsed_time >=80
                scatter(s21a(jj), s21m(jj), 30, data(jj, 7:9), 'filled', 'marker', 's');
            end
            x= datetime(data(jj,2), 'ConvertFrom',  'datenum');
    ...
    % title(strcat("Frequency:", num2str(temporal_data(ii).frequency), "GHz"));
    xlabel('\Delta\angle S_{21}', 'FontAngle', 'italic');
