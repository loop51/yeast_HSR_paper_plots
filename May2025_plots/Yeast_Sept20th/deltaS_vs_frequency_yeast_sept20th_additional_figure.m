...
if ispc
    ...
    figure_save_folder = "C:\Users\mdsaifi\Box\Research\Semesters\Fall2024\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\March2025\yeast_Sept20th\deltaS_vs_frequency";
elseif ismac
    experiment_folder = '/Volumes/Grace Lab/measurement/fall2024/september20th';
    figure_save_folder = '/Volumes/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/september20th/deltaS_vs_frequency_additional_figure'; 
elseif isunix && ~ismac
    experiment_folder = '/media/mdsaifi/Grace Lab/measurement/fall2024/september20th';
    figure_save_folder = '/media/mdsaifi/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/March2025_v2/september20th/deltaS_vs_frequency_additional_figure';
end
...
for ii=1:length(t)
    time_in_minutes(ii) = minutes(t(ii) - min(t));
end
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<4
        h(i) = plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i) = plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), '--o','Color', colors(i,:), 'LineWidth', plot_line_width);
    end
end
grid on;
legend([h(2) h(3) h(5)], {strcat(num2str(time_in_minutes(2),'%.0f'), " mins"), strcat(num2str(time_in_minutes(3),'%.0f'), " mins"), strcat(num2str(time_in_minutes(5),'%.0f'), " mins")}, 'Location', 'northwest');
colorbar;
colormap(parula);
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<4
        h(i)= plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i)= plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '--o','Color', colors(i,:),'LineWidth', plot_line_width);
    end
end
...
colormap(parula);
caxis([minutes(experiment_start_time) minutes(experiment_end_time)]);
% legend([h(2) h(3) h(5)], {strcat(num2str(time_in_minutes(2)), " mins"), strcat(num2str(time_in_minutes(3)), " mins"), strcat(num2str(time_in_minutes(5)), " mins")}, 'Location', 'northeast');
xlabel('f (GHz)');
ylabel('\Delta \angle S11 - \angle S11_{t=0}');
plot_s11a = gca;
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<4
        h(i)= plot(f, deltaS.s21m(i,:)-deltaS.s21m(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i)= plot(f, deltaS.s21m(i,:)-deltaS.s21m(1,:), '--o','Color', colors(i,:), 'LineWidth', plot_line_width);
    end
end
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<4
        h(i)= plot(f, deltaS.s21a(i,:)-deltaS.s21a(1,:), '-o', 'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i)= plot(f, deltaS.s21a(i,:)-deltaS.s21a(1,:), '--o','Color', colors(i,:), 'LineWidth', plot_line_width);
    end
end
...
% figure_s11m.Position = [21.3229 0.4583 3.5 3.5];
...
% figure_s11a.Position = [21.3229 0.4583 3.5 3.5];
figure_s11a.OuterPosition = [21.3229 0.4583 figure_dimension figure_dimension];
...
figure_s21m.OuterPosition = [21.3229 0.4583 figure_dimension figure_dimension];
...
figure_s21a.OuterPosition = [21.3229 0.4583 figure_dimension figure_dimension];
% figure_s21a.Position = [21.3229 0.4583 3.5 3.5];
