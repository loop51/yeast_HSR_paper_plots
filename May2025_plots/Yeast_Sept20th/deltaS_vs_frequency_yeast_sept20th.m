...
if ispc
    ...
    figure_save_folder = 'k:\Yeast_Thermal_effect_paper\yeast HSR paper draft\Results\May2025\september20th\deltaS_vs_frequency';
elseif ismac

elseif isunix && ~ismac
    experiment_folder = '/media/mdsaifi/Grace Lab/measurement/fall2024/september20th';
    figure_save_folder = '/media/mdsaifi/Grace Lab/Yeast_Thermal_effect_paper/yeast HSR paper draft/Results/May2025/september20th/deltaS_vs_frequency';
end
...
assign_iteration_42C = [1,1,1,2, 2,2,3];
assign_set_42C = [3,6,15, 2,5,7,10];
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<3
        h(i) = plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), '-o', 'markersize', 3,'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i) = plot(f, deltaS.s11m(i,:)-deltaS.s11m(1,:), '--o','markersize', 3,'Color', colors(i,:), 'LineWidth', plot_line_width);
    end
end
...
for i=1:size(deltaS.s11m,1)
    hold on;
    if i<3
        h(i)= plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '-s', 'markersize', 3,'Color', colors(i,:), 'LineWidth', plot_line_width);
    else
        h(i)= plot(f, deltaS.s11a(i,:)-deltaS.s11a(1,:), '--s','markersize', 3,'Color', colors(i,:),'LineWidth', plot_line_width);
    end
end
