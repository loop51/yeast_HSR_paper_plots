    ...
    for ii=1:size(processedData,1)
        for jj=1:length(t)
            if t(jj) == processedData(ii,5)
                ...
                [d_s11r d_s11i] = pol2cart(deg2rad(detrended.s11a(jj)), d_s11m);
                [d_s21r d_s21i] = pol2cart(deg2rad(detrended.s21a(jj)), d_s21m);

                s11m = 10^(s.s11m(jj)/20);
                s21m = 10^(s.s21m(jj)/20);
                [s11r s11i] = pol2cart(deg2rad(s.s11a(jj)), s11m);
                [s21r s21i] = pol2cart(deg2rad(s.s21a(jj)), s21m);
...
function filter_adjusment(dl, order, framelen) % here dl is folder/path where all the sets are
    ...
    for i=1:length(dl_list)
        ...
        try
            ...
            hold on;
            for ii = 1:length(dS.s11m)
                text((T(ii)), peaks.s11m(ii), {num2str(dS.s11m(ii)), num2str((T(ii)))});
            end
            ...
            for ii = 1:length(dS.s11a)
                text((T(ii)), peaks.s11a(ii), {num2str(dS.s11a(ii)), num2str((T(ii)))});
            end
            hold on;
            grid on;
            grid minor;
            ...
            for ii = 1:length(dS.s21m)
                text((T(ii)), peaks.s21m(ii), {num2str(dS.s21m(ii)), num2str((T(ii)))});
            end
            hold on;
            grid on;
            grid minor;
            ...
            for ii = 1:length(dS.s21a)
                text((T(ii)), peaks.s21a(ii), {num2str(dS.s21a(ii)), num2str((T(ii)))});
            end
            hold on;
            grid on;
            grid minor;
